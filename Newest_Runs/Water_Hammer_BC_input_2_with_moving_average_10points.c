...
int n_time;
real VelStored[10000]; /*careful here, assumes that no more than 10,000 timesteps*/
...
DEFINE_PROFILE(VelBC, thread, position)
{
	...
	if (n_time >= 3){
		...
		begin_f_loop(face, thread)
		{
			F_PROFILE(face, thread, position) = (VelStored[n_time]+VelStored[n_time-1]+VelStored[n_time-2]+VelStored[n_time-3]+VelStored[n_time-4]+VelStored[n_time-5]+VelStored[n_time-6]])/7;
		}
		end_f_loop(face, thread)
	}
