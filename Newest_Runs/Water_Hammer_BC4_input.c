...
DEFINE_PROFILE(VelBC, thread, position)
{
	...
	if ( CURRENT_TIME == 0 )
	{
		...
		for(x = 0; x < nx; x++ ){
			V[x] = Vinit;
			Hold[x] = PBOP + dx*pow(Vinit,2.0)*f/2.0/g/D*(nx-x-1);/*((nx/1.-x/1.)/nx*Length)*pow(Vinit,2)*f/2/g/D;*/
			Message("Hold[x] =  %lf \n",Hold[x]);
		}
	...
	if ( first_iteration )
		{
			...
			if (test < 0.5){
				...
				Message("VnewBoundary[x] =  %lf \n",C*(Hold[nx-1]-Hold[nx-2])+K*(V[nx-2]*abs(V[nx-2])));
				Message("VnewBoundary[x] =  %lf \n",C*(Hold[nx-1]-Hold[nx-2])+K*(V[nx-2]*abs(V[nx-2])));
				Message("VnewBoundary[x] =  %lf \n",C*(Hold[nx-1]-Hold[nx-2]+fraction));
				Message("VnewBoundary[x] =  %lf \n",C*(Hold[nx-1]-Hold[nx-2]-fraction));
			}
